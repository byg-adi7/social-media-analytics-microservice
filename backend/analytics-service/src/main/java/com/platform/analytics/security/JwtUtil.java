...
/**
 * Verifies JWTs issued by Supabase Auth locally. Ported from the former
 * standalone Auth Service (which had shrunk to nothing but this one class)
 * so every request no longer needs a network hop just to check the token -
 * this class is the one place that verifies a token is genuine.
 */
@Component
